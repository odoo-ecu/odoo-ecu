...
class EcWaybill(models.Model):
    ...
    date = fields.Datetime('Transport Date', default=fields.Datetime.now, copy=False, required=True)
    date_due = fields.Datetime('Transport Due Date', default=fields.Datetime.now, copy=False, required=True)
    ...
    auth_number = fields.Char("Authorization Number", readonly=True, compute="_get_authorization")
    auth_date = fields.Datetime("Authorization Date", readonly=True, compute="_get_authorization")
    ...
    def _compute_l10n_ec_waybill_l_seq(self):
        for waybill in self:
            ...
            self.env.cr.execute(sql, {'warehouse_id': waybill.location_id.warehouse_id.id or -1})
            seq = (self.env.cr.fetchone() or [None])[0]
            waybill.l10n_ec_waybill_last_sequence = seq or "/"
    ...
    def _assign_l10n_ec_waybill_document_number(self):
        ...
        for waybill in self:
            waybill._compute_l10n_ec_waybill_l_seq()
    ...
    def _compute_l10n_ec_waybill_ak(self):
        ...
        issuing_date = fields.Datetime.context_timestamp(self, self.date)
        ...
        try:
            ...
            raise ValidationError("Error validating access key: {}".format(e))
