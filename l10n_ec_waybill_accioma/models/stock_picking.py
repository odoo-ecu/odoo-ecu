...
class StockPicking(models.Model):
    ...
    def _get_waybill_partner_data(self):
        ...
        for picking in self:
            if picking.partner_id:
                ...
                picking.l10n_ec_waybill_partner_street = warehouse.partner_id.street if warehouse else picking.company_id.street


    @api.depends('sale_id', 'sale_id.partner_id', 'sale_id.invoice_ids', 'company_id')
    def _get_invoice_name(self):
        for picking in self:
            if picking.sale_id:
                ...
                picking.l10n_ec_waybill_partner_street = picking.sale_id.partner_id.street

                invoices = picking.sale_id.invoice_ids.filtered(lambda x: x.state == 'posted')
                if invoices:
                    ...
                    picking.l10n_ec_waybill_invoice_auth = invoices[0].l10n_ec_edi_access_key or ""
                else:
                    picking.l10n_ec_waybill_invoice_number = ""
                    picking.l10n_ec_waybill_invoice_date = False
    ...
    def _check_waybill_document_number(self):
        ...
        for waybill in self.filtered(lambda waybill: waybill.is_l10n_ec_waybill):
    ...
    @api.depends('is_l10n_ec_waybill', 'l10n_ec_waybill_warehouse_id')
    def _compute_l10n_ec_waybill_l_seq(self):
        for picking in self:
            ...
            self.env.cr.execute(sql, {'warehouse_id': picking.l10n_ec_waybill_warehouse_id.id or -1 })
            seq = (self.env.cr.fetchone() or [None])[0]
            picking.l10n_ec_waybill_last_sequence = seq or "/"
